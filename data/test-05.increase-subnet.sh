...
do 
    ...
    cp data/nmap.results.txt ../$RESULTS_DIR/nmap.results.num_subnet_${CURRENT_NUM_SUBNET}.txt
    sleep $SECONDS_BETWEEN_STEPS
    ...
    echo "| ${CURRENT_NUM_SUBNET} | $(bc <<< "$CURRENT_NUM_SUBNET*$NUM_SUBNET_HOSTS") | $(cat data/nmap.results.txt | grep "scanned in" | cut -d " " -f 19) | $(cat data/nparallel.log | grep "Finished" | cut -d " " -f 6) | $EQUAL_NUM_PORTS | $NUM_NMAP_PORTS | $NUM_NPARALLEL_PORTS | " >> ../$test_name.results.md
